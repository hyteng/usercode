...
void RPCCosmicSeedrecHitFinder::fillrecHits() {
    ...
    if(isLayerset == false || isConfigured == false || isOutputset == false || isInputset == false || isEdgeset == false) {
        if(debug) cout << "Not set the IO or not configured yet" << endl;
        return;
    } 
    ...
    if(LayersinRPC.size() == 0) {
        if(debug) cout << "Not set with any layers" << endl;
        LayersinRPC.clear();
        therecHits.clear();
        isLayerset = false;
    }
...
bool RPCCosmicSeedrecHitFinder::complete(const GlobalVector& lastSegment, const MuonRecHitPointer& lastrecHitRef) {
    ...
    for(unsigned int i = 0; i < RPCLayerNumber; i++)
        for(MuonRecHitContainer::const_iterator it = AllrecHits[i].begin(); it != AllrecHits[i].end(); it++) {
            ...
            if(debug) cout << "Finding recHits from " << i << " th layer" << endl;
            ...
            if(debug) cout << "Candidate recHit's position: " << currentPosition.x() << ", " << currentPosition.y() << ", " << currentPosition.z() << ". BX : " << currentBX << endl;
            ...
            if(isOuterLayerfilled == false) {
                ...
                if(debug) cout << "1st recHit's global position: " << currentPosition.x() << ", " << currentPosition.y() << ", " << currentPosition.z() << ". BX: " << currentBX << endl;
                isrecHitsfound = true;
                ...
                if(debug) cout << "Check isinsideRegion: " << isinsideRegion << endl;
                if(!isinsideRegion)
                    continue;

                // If cross the edge the recHit should belong to another seed
                bool iscrossanyEdge = iscorssEdge(lastrecHitRef, *it);
                if(debug) cout << "Check iscrossanyEdge: " << iscrossanyEdge << endl;
                if(iscrossanyEdge)
                    continue;
                ...
                if(debug) cout << (NumberinSeed + 1) << "th recHit's global position: " << currentPosition.x() << ", " << currentPosition.y() << ", " << currentPosition.z() << ". BX: " << currentBX << endl;
                isrecHitsfound = true;
                therecHits.push_back(*it);
                ...
                if(findNext == false && therecHits.size() > 3) {
                    for(ConstMuonRecHitContainer::const_iterator iter = therecHits.begin(); iter != therecHits.end(); iter++)
                        if(debug) cout << "Find recHit in seed candidate : " << (*iter)->globalPosition().x() << ", " << (*iter)->globalPosition().y() << ", " << (*iter)->globalPosition().z() << endl;
                    checkandfill();
                }
...
bool RPCCosmicSeedrecHitFinder::iscorssEdge(const MuonRecHitPointer& lastrecHitRef, const MuonRecHitPointer& currentrecHitRef) {
    ...
    for(std::vector<BoundPlane>::const_iterator it = innerBounds.begin(); it != innerBounds.end(); it++) {
        ...
        //if(debug) cout << "Side of currentPosition: " << TestSide0 << ", Side of lastPosition: " << TestSide1 << ", Side of middlePosition: " << TestSide << endl;
